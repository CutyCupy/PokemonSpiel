...
//		gController.getGameFrame().getFightPanel().updatePanels();
		if (gController.checkPlayerDead()) {
			...
			gController.getGameFrame().getFightPanel().addText(enemy.getName() + " wurde besiegt!");
			gController.getGameFrame().getFightPanel().addText(player.getName() + " erh�lt "
					+ (int) ((enemy.getStats().getLevel() * 1.25 * 100) / 7));
			gController.getGameFrame().getFightPanel().updatePanels();
			if(!gController.winFight(calculateXP())) {
	...
	public boolean playerHit(Move playerMove) {
		if (rng.nextFloat() < playerMove.getAccuracy() && playerMove.getCurrentPP() > 0) {
			playerMove.reducePP();
...
//					gController.getGameFrame().getFightPanel().updatePanels();
		...
		for(int i = 0; i < pokemon.getStats().getStats().length; i++) {
			...
			if(change < 0) {
				if(pokemon.getStats().decreaseStat(i, Math.abs(change))) {
					if(change == -1) {
						gController.getGameFrame().getFightPanel().addText(Stats.STAT_NAMES[i] + " von " + pokemon.getName() + " wurde gesenkt!");
					} else if(change <= -2) {
						gController.getGameFrame().getFightPanel().addText(Stats.STAT_NAMES[i] + " von " + pokemon.getName() + " wurde sehr stark gesenkt!");
					}
				}
			} else if(change > 0) {
				if(pokemon.getStats().increaseStat(i, change)) {
					if(change == 1) {
						gController.getGameFrame().getFightPanel().addText(Stats.STAT_NAMES[i] + " von " + pokemon.getName() + " ist gestiegen!");
					} else if(change >= 2) {
						gController.getGameFrame().getFightPanel().addText(Stats.STAT_NAMES[i] + " von " + pokemon.getName() + " ist sehr stark gestiegen!");
					}
				}
			}
		}
	...
	private void damageCalculation(Pokemon attacker, Pokemon defense, Move usedMove) {
		...
		if(damage > 0 || usedMove.getDamageClass() != DamageClass.NO_DAMAGE) {
			...
			defenderStats.loseHP((int) (weakness * ((attackerStats.getLevel() * (2 / 5.0) + 2) * damage * (atk / (50.0 * def)) + 2) * crit
