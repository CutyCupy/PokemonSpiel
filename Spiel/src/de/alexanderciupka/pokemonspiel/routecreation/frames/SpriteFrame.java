...
public class SpriteFrame extends JFrame {
	...
	public SpriteFrame() {
		setBounds(1280, 0, 640, 600);

		JPanel panel = new JPanel();
		panel.setLayout(new GridLayout(0, 10));

		ArrayList<String> sprites = new ArrayList<>(GameController.getInstance().getRouteAnalyzer().getSpriteNames());
		...
		for (String sprite : sprites) {
			BufferedImage img = GameController.getInstance().getRouteAnalyzer().getSpriteByName(sprite);
			System.out.println(img.getWidth() + "x" + img.getHeight());
			JLabel label = new JLabel(
					new ImageIcon(img.getHeight() > GameFrame.GRID_SIZE || img.getWidth() > GameFrame.GRID_SIZE
							? img.getScaledInstance(GameFrame.GRID_SIZE, GameFrame.GRID_SIZE, Image.SCALE_SMOOTH)
							: img));
