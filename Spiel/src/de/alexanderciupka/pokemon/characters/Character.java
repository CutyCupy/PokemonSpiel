...
import de.alexanderciupka.pokemon.map.entities.types.ItemEntity;
...
//		this.id = "";
	...
	public void changePosition(Direction direction, boolean waiting) {
		if (this.controllable) {
			...
			this.setSurfing(this.getCurrentRoute().getEntity(this.currentPosition.x,
					this.currentPosition.y) instanceof WaterEntity);
			this.onMovement();
		}
	}
	...
	public boolean isTrainer() {
		return this.team != null && this.team.getAmmount() > 0;
	}
	...
	public JsonObject getSaveData() {
		...
		currentData.addProperty("route", this.currentRoute.getId());
		currentData.addProperty("x", this.currentPosition.x);
		currentData.addProperty("y", this.currentPosition.y);
		currentData.addProperty("direction", this.currentDirection.name());
		data.add("current_data", currentData);

		JsonObject originalData = new JsonObject();
		originalData.addProperty("x", this.originalPosition.x);
		originalData.addProperty("y", this.originalPosition.y);
		originalData.addProperty("direction", this.originalDirection.name());
		data.add("original_data", originalData);

		data.addProperty("sprite", this.spriteName);
		data.addProperty("step_counter", this.stepCounter);

		if (this.isTrainer()) {
			JsonObject trainerData = new JsonObject();
			trainerData.add("team", this.getTeam().getSaveData());
			trainerData.addProperty("money", this.money);
			trainerData.addProperty("range", this.range);
			...
			trainerData.add("items", items);
			data.add("trainer_data", trainerData);
		}
		return data;
	}

	public boolean importSaveData(JsonObject saveData) {
		System.out.println(saveData.get("id") + " - " + this.id);
		if (this.id == null || this.id.equals(saveData.get("id").getAsString())) {
			...
			if (saveData.has("trainer_data")) {
				JsonObject trainerData = saveData.get("trainer_data").getAsJsonObject();
				if (this.team == null) {
					this.team = new Team(this);
				}
				this.team.importSaveData(trainerData.get("team").getAsJsonArray());
				this.money = trainerData.get("money").getAsLong();
				this.range = trainerData.get("range").getAsInt();
				this.items = new HashMap<>();
				for (JsonElement e : trainerData.get("items").getAsJsonArray()) {
					JsonObject item = e.getAsJsonObject();
					this.items.put(item.get("id").getAsInt(), item.get("amount").getAsInt());
				}
			}
//			this.setCurrentRoute(this.gController.getRouteAnalyzer()
//					.getRouteById(saveData.get("current_data").getAsJsonObject().get("route").getAsString()));
			System.err.println(this.getCurrentRoute());
			this.setCurrentPosition(saveData.get("current_data").getAsJsonObject().get("x").getAsInt(),
					saveData.get("current_data").getAsJsonObject().get("y").getAsInt());
			this.setOriginalPosition(new Point(saveData.get("original_data").getAsJsonObject().get("x").getAsInt(),
					saveData.get("original_data").getAsJsonObject().get("y").getAsInt()));
			try {
				this.setCurrentDirection(Direction
						.valueOf(saveData.get("current_data").getAsJsonObject().get("direction").getAsString()));
			} catch (Exception e) {
				this.setCurrentDirection(Direction.DOWN);
			}
			try {
				this.setOriginalDirection(Direction
						.valueOf(saveData.get("original_data").getAsJsonObject().get("direction").getAsString()));
			} catch (Exception e) {
				this.setOriginalDirection(Direction.DOWN);
			}
			this.setSurfing(
					this.currentRoute.getEntity(this.currentPosition.x, this.currentPosition.y) instanceof WaterEntity);
			this.setCharacterImage(saveData.get("sprite").getAsString());
	...
	public void run() {
		...
		this.currentRoute.getEntity(this.currentPosition.x, this.currentPosition.y).onStep(this);
		this.moving = false;
	}
	...
	public Direction[] moveTowards(int x, int y) {
		...
		for (int i = 0; i < distance.length; i++) {
			...
			predecessor[i] = -1;
			nodes.put(i,
					this.currentRoute.getEntity(i % this.currentRoute.getWidth(), i / this.currentRoute.getWidth()));
		}
		distance[start] = 0;
	...
	public String useItem(Integer item) {
		...
		boolean usable = this.gController.isFighting() && info.hasAttribute(Items.ATTR_USABLE_IN_BATTLE, item)
				|| (!this.gController.isFighting() && info.hasAttribute(Items.ATTR_USABLE_OVERWORLD, item));
		if (usable) {
			switch (item) {
			...
			case Items.POKEBALL:
				...
				return null;
			case Items.SCHUTZ:
			case Items.SUPERSCHUTZ:
			case Items.TOP_SCHUTZ:
				if (this.isProtected()) {
					return Items.USELESS;
				} else {
					int steps = item == Items.SCHUTZ ? 100 : (item == Items.SUPERSCHUTZ ? 200 : 250);
					this.protectedSteps = this.stepCounter + steps;
					return ("Du bist jetzt für " + steps + " Schritte vor wilden Pokemon geschützt!");
				}
			case Items.CUTTER:
			case Items.FLY:
			case Items.SURFBOARD:
			case Items.STRENGTH:
			case Items.WATERFALL:
			case Items.ROCKSMASH:
				if (this instanceof Player) {
					Entity interaction = this.currentRoute.getEntity(this.getInteractionPoint().y,
							this.getInteractionPoint().x);
					if (interaction != null && !interaction.useVM((Player) this, item)) {
						return Items.USELESS;
					}
				}
				break;
			case Items.ANGEL:
			case Items.PROFIANGEL:
			case Items.SUPERANGEL:
				// TODO Angeln
			case Items.ITEMRADAR:
				for (int x = -4; x < 5; x++) {
					for (int y = -4; y < 5; y++) {
						Entity e = this.currentRoute.getEntity(this.currentPosition.x + x, this.currentPosition.y + y);
						if (e instanceof ItemEntity && ((ItemEntity) e).getItem() != Items.KEINS
								&& ((ItemEntity) e).isHidden()) {
							// TODO: Success sound + hint
						}
					}
				}
			default:
				// TODO: Pokemon selection
			}
		}
		return Items.USELESS;
	}
	...
	// case REPEL:
	// if (this.isProtected()) {
	// this.gController.getGameFrame().addDialogue("Es wurde bereits ein Schutz
	// eingesetzt!");
	// } else {
	// this.protectedSteps = this.stepCounter + i.getValue();
	// this.gController.getGameFrame()
	// .addDialogue("Du bist jetzt für " + i.getValue() + " Schritte vor wilden
	// Pokemon geschützt!");
	// result = true;
	// }
	// case HYPERBALL:
